...
SELECT Magv, Hotengv, Tenkhoa
FROM TBLGiangVien, TBLKhoa
WHERE TBLGiangVien.Makhoa = TBLKhoa.Makhoa AND TBLKhoa.Tenkhoa = 'Dia ly va QLTN'

--3.	Cho biết số sinh viên của khoa ‘CONG NGHE SINH HOC’
SELECT COUNT(*) AS N'Số sinh viên của khoa Công nghệ Sinh học'
FROM TBLSinhVien, TBLKhoa
WHERE TBLSinhVien.Makhoa = TBLKhoa.Makhoa AND TBLKhoa.Tenkhoa = 'Cong nghe Sinh hoc'
...
FROM TBLGiangVien, TBLKhoa
WHERE TBLGiangVien.Makhoa = TBLKhoa.Makhoa AND TBLKhoa.Tenkhoa = 'Cong nghe Sinh hoc'

--6.	Cho biết thông tin về sinh viên không tham gia thực tập
SELECT *
FROM TBLSinhVien
WHERE Masv NOT IN (SELECT Masv FROM TBLHuongDan)
...
WHERE TBLDeTai.Madt = TBLHuongDan.Madt AND TBLGiangVien.Magv = TBLHuongDan.Magv AND TBLGiangVien.Hotengv = 'Tran Son'

--2.	Cho biết tên đề tài không có sinh viên nào thực tập
SELECT Tendt
FROM TBLDeTai
WHERE Tendt NOT IN (SELECT Tendt FROM TBLDeTai, TBLHuongDan WHERE TBLDeTai.Madt = TBLHuongDan.Madt)
...
FROM TBLGiangVien, TBLKhoa, TBLHuongDan
...
FROM TBLSinhVien, TBLHuongDan, TBLKhoa
WHERE TBLSinhVien.Masv = TBLHuongDan.Masv AND TBLSinhVien.Makhoa = TBLKhoa.Makhoa AND TBLKhoa.Tenkhoa = 'Dia ly va QLTN'
...
WHERE TBLSinhVien.Masv = TBLHuongDan.Masv AND TBLHuongDan.Madt = TBLDeTai.Madt AND TBLDeTai.Noithuctap = Quequan
